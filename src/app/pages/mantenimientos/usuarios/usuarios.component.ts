...
export class UsuariosComponent implements OnInit, OnDestroy {
  ...
  cargarUsuarios(){
    ...
    this.usuarioService.cargarUsuarios(this.desde, this.limite)
        .subscribe(({registros, usuarios}) => {
          this.totalUsuarios = registros;
  ...
  eliminarUsuario(usuario: Usuario){
    ...
    if(usuario.uid === this.usuarioService.uid){
      return Swal.fire('Error','No puede borrar el usuario actual','error');
    }

    Swal.fire({
      title: 'Borrar usuario?',
      text: `Esta seguro que quiere borrar a ${usuario.nombre}`,
      icon: 'question',
